...
namespace MppcDec
{
    public class MppcUnpacker : StreamEncoder
    {
        ...
        public MppcUnpacker()
        {
            ...
            unpackedBytes = new List<byte>(16 * 1024);
        }

        public void Unpack(byte packedByte, DataStream unpackedChunk)
        {
            packedBytes.Add(packedByte);

            if (unpackedBytes.Count >= 10240)
                unpackedBytes.RemoveRange(0, 8192);
        ...
        public override void Encode(byte[] buffer, int offset, int length, DataStream outputStream)
        {
            ...
            {
                Unpack(buffer[offset + i], outputStream);
            }

        }

        private void Copy(int shift, int size, DataStream unpackedChunkData)
        {
